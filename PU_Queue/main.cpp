...
void addTest3(PUQueue *q, Request *r, int t, int s);
std::tuple<int,int> getTest3(PUQueue *q, int t, int s);
...
PUQueue *createPUQueueFromRandomValues(int numRequestsToGenerate, int numTestsToRun) {
    auto *queue = new PUQueue();
    srandom(getpid());
    ...
    while ( numTestsRun < numTestsToRun ) {
        ...
        if (numRequestsGenerated < numRequestsToGenerate && actionRange >= ADD_A_REQUEST && actionRange < GET_A_REQUEST) {
            ...
            request = generateRandomRequest();
            std::cout << "Adding the following request to the PUQueue.\n";
            request->print();
            ...
            if (queue->empty())
                std::cout << "PUQueue is empty.\n";
            else {
                request = queue->getRequest();
                std::cout << "Removed the following request from the PUQueue via getRequest.\n";
                request->print();
            ...
            if (numRequestsInTheQueue != 0 && queue->empty()) {
                std::cout << "PUQueue::empty() returns true, but the queue should have " <<
                          numRequestsInTheQueue << " elements in it." << std::endl;
                exit(1);
            } else if (numRequestsInTheQueue == 0 && !queue->empty()) {
                std::cout << "PUQueue::empty() returns false, but the queue should be empty." << std::endl;
                exit(1);
            }
        }
        numTestsRun++;
    ...
    while(! queue->empty() ) {
        std::cout << "Removed the following request from the PUQueue after having tested the queue.\n";
    ...
    if( numRequestsInTheQueue > 0 ) {
        std::cout << "In the process of testing, the PUQueue lost " << numRequestsInTheQueue <<
            ". This means the queue contains a bug!\n";
    } else if(numRequestsInTheQueue < 0) {
        std::cout << "In the process of testing, the PUQueue did not remove " << -numRequestsInTheQueue <<
...
int main(int argc, char *argv[]) {
    ...
    if( randomTest )
        queue = createPUQueueFromRandomValues(60, 20);
    else
    ...
    addTest3(q, r1, 0, 0);
    pair = getTest3(q, std::get<0>(tup), std::get<1>(tup));
    addTest3(q, r2, std::get<0>(pair), std::get<1>(pair));
    addTest3(q, r3, std::get<0>(pair), std::get<1>(pair));
    pair = getTest3(q, std::get<0>(pair), std::get<1>(pair));
    pair = getTest3(q, std::get<0>(pair), std::get<1>(pair));
    addTest3(q, r4, std::get<0>(pair), std::get<1>(pair));
    addTest3(q, r5, std::get<0>(pair), std::get<1>(pair));
    addTest3(q, r6, std::get<0>(pair), std::get<1>(pair));
    pair = getTest3(q, std::get<0>(pair), std::get<1>(pair));
    addTest3(q, r7, std::get<0>(pair), std::get<1>(pair));
    pair = getTest3(q, std::get<0>(pair), std::get<1>(pair));
    pair = getTest3(q, std::get<0>(pair), std::get<1>(pair));
